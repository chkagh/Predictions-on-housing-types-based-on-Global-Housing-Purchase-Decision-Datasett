# -*- coding: utf-8 -*-
"""Kopia notatnika Python in ML - Lab 5.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1ex3mF0CiupYdold3GT3UyiB5AgoiLho7
"""
...
#data = pd.read_csv('/content/drive/MyDrive/Colab Notebooks/Lab_2/global_house_purchase_dataset.csv')
data = pd.read_csv('/content/drive/MyDrive/PMLDS_files/global_house_purchase_dataset.csv')
...
X_temp, X_test, y_temp, y_train = train_test_split(X, y, test_size=0.3, random_state=1)
...
print("Training RandomForestClassifier with scaled features...")

clf_rf = Pipeline([("scaler", StandardScaler()), ("rf", RandomForestClassifier(random_state=1, n_jobs=-1))])
...
acc_val_rf = accuracy_score(y_valid, y_pred_rf)

print('Model: RandomForestClassifier')
print(f'Accuracy (train set): {acc_train_rf:.4f}')
print(f'Accuracy (validation set): {acc_val_rf:.4f}')
...
selected_features = ['rooms', 'property_size_sqft']

X_train_svm_2features = X_train[selected_features]
X_valid_svm_2features = X_valid[selected_features]
X_test_svm_2features = X_test[selected_features]

print(f"Training SVC model with features: {selected_features}")

# Create an SVM pipeline with StandardScaler
svm_clf = Pipeline([("scaler", StandardScaler()), ("svm", SVC(random_state=1))])

# Fit the model
svm_clf.fit(X_train_svm_2features, y_train)

# Make predictions
y_pred_train_svm = svm_clf.predict(X_train_svm_2features)
y_pred_valid_svm = svm_clf.predict(X_valid_svm_2features)

# Calculate and print accuracy scores
acc_train_svm = accuracy_score(y_train, y_pred_train_svm)
acc_valid_svm = accuracy_score(y_valid, y_pred_valid_svm)

print(f'\nModel: SVM (Support Vector Classifier) with {selected_features}')
print(f'Accuracy (train set): {acc_train_svm:.4f}')
print(f'Accuracy (validation set): {acc_valid_svm:.4f}')

sns.set_theme()
sns.scatterplot(x=data['rooms'], y=data['property_size_sqft'],hue=data['property_type'])
plt.title('Number of Rooms vs Property Size')
plt.xlabel('Rooms')
plt.ylabel('Property Size - sqft')
plt.legend(title='Property Type', bbox_to_anchor=(1.05, 1), loc='upper left')
plt.show()

apartment_data = data[data['property_type'] == 'Apartment']

plt.figure(figsize=(10, 6))
sns.scatterplot(x=apartment_data['rooms'], y=apartment_data['property_size_sqft'], color='orange')
plt.title('Number of Rooms vs Property Size for Apartments')
plt.xlabel('Rooms')
plt.ylabel('Property Size (sqft)')
plt.show()
...
for p_type in data['property_type'].unique():
    type_data = data[data['property_type'] == p_type]
    # Sample up to 50 rows, or fewer if less than 50 are available for that type
    sampled_type = type_data.sample(n=min(len(type_data), 50), random_state=42)
    sampled_data = pd.concat([sampled_data, sampled_type])

plt.figure(figsize=(12, 8))
sns.scatterplot(data=sampled_data, x='rooms', y='property_size_sqft', hue='property_type', s=100, alpha=0.7)
plt.title('Number of Rooms vs Property Size for All Property Types (Sampled)')
plt.xlabel('Rooms')
plt.ylabel('Property Size (sqft)')
plt.legend(title='Property Type', bbox_to_anchor=(1.05, 1), loc='upper left')
plt.tight_layout()
...
sns.scatterplot(x=data['property_type'], y=data['rooms'])
plt.title('Property Type vs Rooms')
plt.xlabel('Property Type')
plt.ylabel('Rooms')
plt.show()

sns.scatterplot(x=data['property_type'], y=data['property_size_sqft'])
plt.title('Property Type vs Rooms')
plt.xlabel('Property Type')
plt.show()
...
y_pred_cm = svm_clf.predict(X_valid_svm_2features)

# Wygenerowanie confusion matrix
ConfusionMatrixDisplay.from_predictions(y_valid, y_pred_cm)

# Poprawa czytelności osi
